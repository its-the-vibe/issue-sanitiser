...
1. **Extract issue details**: Parse the URL to get the owner, repository, and issue number, then fetch the issue using `gh issue view` command
...
1. **Parse the GitHub issue URL** to extract:
   ...
   - Repository name
   - Issue number
2. **Fetch the issue** using `gh issue view <issue-url>`
